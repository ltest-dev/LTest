...
let pp_status formatter status =
  Format.pp_print_string formatter (if !Instrument.at=Cil_types.Req then string_of_status status else string_of_statusAT status)
...
let load_add data idstr statusstr tag origin_loc current_loc emitter exec_time extra replace line =
  try
    let id = int_of_string idstr in
    let status = status_of_string statusstr in
    if replace then
      Hashtbl.replace data id {status; tag; current_loc; origin_loc; emitter; exec_time; extra}
    else if not (Hashtbl.mem data id) then
      Hashtbl.add data id {status; tag; current_loc; origin_loc; emitter; exec_time; extra}
    else
...
let rec load_stream replace linenum input data =
  ...
  | Some line ->
    if line.[0] != '#' && String.length line > 0 then begin
      ...
      | idstr :: statusstr :: tag :: origin_loc :: current_loc :: emitter :: exec_time :: extra ->
        load_add data idstr statusstr tag origin_loc current_loc emitter (float_of_string exec_time) extra replace linenum
...
let store data filepath =
  if Options.Backup.get () then backup filepath;
  let output = open_out filepath in
  let f id {status; tag; current_loc; origin_loc; emitter; exec_time; extra} acc =
    (id, (String.concat "," (string_of_int id :: string_of_status status :: tag :: origin_loc :: current_loc :: emitter :: (string_of_float exec_time) :: extra ))) :: acc
  in
...
let storeAT data filepath =
  ...
  let output = open_out filepath in
  let f id {status; tag; current_loc; origin_loc; emitter; exec_time; extra} acc =
    (id, (String.concat ", " (string_of_int id :: string_of_statusAT status :: tag :: origin_loc :: current_loc :: emitter :: (string_of_float exec_time) :: extra ))) :: acc
  in
  let l = Hashtbl.fold f data [] in
  let l = List.sort compare l in
  output_string output data_header;
...
let merge_status_info ?(force=false) label status emitter time =
  ...
  if force || new_status <> label.status then
    begin
      ...
      Extlib.may (fun time -> label.exec_time <- time) time
    end
...
let default () =
  { status = Unknown; tag = ""; origin_loc = ""; current_loc = ""; emitter = ""; exec_time = 0.0; extra = [] }
